...
from tools.utils import GetExceptionTraceBack
import copy

# impedance value invalid (electrode disconnected)
CHAMP_IMP_INVALID = 999900  # INT_MAX

def TranslateUnits(unit):
    u = unit
    if unit == "":
        u = "µV"
    if unit == "unknown":
        u = "?"
    if unit == "microvolts" or unit == "uV":
        u = "µV"
    return u
...
class ChannelGroup:
    ...
    '''
    (ALL, EEG, AUX, EPP, BIP) = list(range(5))
    Name = ["ALL", "EEG", "AUX", "EPP", "BIP"]
...
class EEG_ChannelProperties(object):
    ...
    def __init__(self, name):
        ...
        self.amplifier = 0                  #: zero based amplifier number
        self.module = 0                     #: zero based module number
        self.modulechannel = 0              #: zero based physical channel number (within a module)
        
    def __deepcopy__(self, memodict={}):
        ''' fast deep copy of channel properties
        '''
        copy_obj = EEG_ChannelProperties("")
        copy_obj.xmlVersion = self.xmlVersion            
        copy_obj.input = self.input                
        copy_obj.inputgroup = self.inputgroup
        copy_obj.enable = self.enable       
        copy_obj.name = self.name
        copy_obj.refname = self.refname       
        copy_obj.group = self.group
        copy_obj.lowpass = self.lowpass
        copy_obj.highpass = self.highpass
        copy_obj.notchfilter = self.notchfilter 
        copy_obj.isReference = self.isReference    
        copy_obj.color = self.color  
        copy_obj.unit = self.unit    
        copy_obj.amplifier = self.amplifier
        copy_obj.module = self.module       
        copy_obj.modulechannel = self.modulechannel       
        return copy_obj 
...
class EEG_Marker(object):
    ...
    def __deepcopy__(self, memodict={}):
        ''' fast deep copy of marker properties
        '''
        copy_obj = EEG_Marker()
        copy_obj.position = self.position
        copy_obj.points = self.points
        copy_obj.type = self.type
        copy_obj.description = self.description
        copy_obj.invisible = self.invisible
        copy_obj.channel = self.channel
        copy_obj.date = self.date
        return copy_obj
...
class EEG_DataBlock(object):
    ...
    def __init__(self, eeg=32, aux=8):
        ...
        self.trigger_channel = np.zeros((1, 1000), np.uint32)   #: trigger and digital values
        ...
        self.digitalChannelDescription = [] #: description of digital/trigger channels
        self.timestamp_channel = np.zeros((1, 10), np.double)   #: timestamp values
    ...
    def __copy__(self):
        ...
        copy_obj.impedances = self.impedances
        copy_obj.block_time = self.block_time
        ...
        copy_obj.digitalChannelDescription = self.digitalChannelDescription
        copy_obj.timestamp_channel = self.timestamp_channel
        return copy_obj 
...
class AmplifierModuleDescription(object):
    ''' Description of amplifier modules (e.g. module type, serial number, hardware revision ...)
    '''
    def __init__(self, type, serialNumber, hardwareRevision, firmwareVersion):
        self.Type = type
...
class AmplifierDescription(object):
    ''' Description of the connected amplifier and its modules
    '''
    def __init__(self, amplifierFamily, libraryVersion):
        self.Family = amplifierFamily
    ...
    def __repr__(self):
        ...
        for m in self.Modules:
            md = "%s SN:%s HW:%s FW:%s\n"%(m.Type, m.SerialNumber, m.HardwareRevision, m.FirmwareVersion)
            d += md
        d += "%s"%self.LibraryVersion
        return d
...
class ModuleBase(Qt.QObject):
    ...
    '''
    eventToParent = Qt.Signal(object)
    eventToReceiver = Qt.Signal(object)
    ...
    def add_receiver(self, receiver):
        ...
        receiver.eventToParent.connect(self.receiver_event, Qt.Qt.QueuedConnection)
        # tell the receiver to get events from parent
        self.eventToReceiver.connect(receiver.parent_event, Qt.Qt.QueuedConnection)
    ...
    def parent_event(self, event):
        ...
        self.eventToReceiver.emit(cevent)
    ...
    def receiver_event(self, event):
        ...
        cevent = copy.deepcopy(event)
        self.eventToParent.emit(cevent)
    ...
    def send_event(self, event):
        ...
        @param event: ModuleEvent object
        '''
        revent = copy.deepcopy(event)
        self.eventToReceiver.emit(revent)
        pevent = copy.deepcopy(event)
        self.eventToParent.emit(pevent)
